...
function updateSummary(bestState = lastBest) {
  ...
  secondaryUnbalanceEl.textContent = formatMA(current.unbalanceMA / current.system.ctPrimary);
  bestUnbalanceEl.textContent = formatMA(best.unbalanceMA);
  bestSecondaryUnbalanceEl.textContent = formatMA(best.unbalanceMA / best.system.ctPrimary);
...
function applyLoadedValues(values, sourceName) {
  if (values.length < TOTAL_CAPS || values.some((value) => !Number.isFinite(value))) {
    setFileStatus(`Could not find ${TOTAL_CAPS} capacitance values in ${sourceName}.`, true);
    return;
  }
  ...
  renderLayout();
  updateSummary();
  setFileStatus(`Loaded ${TOTAL_CAPS} capacitance values from ${sourceName} into ${currentPhase}.`);
}
...
function createRecord(bestState) {
  ...
  return {
    title: `132kV H Type 48 Capacitor Bank Unbalanced Current - ${currentPhase}`,
    phase: currentPhase,
    ...
    beforeSecondaryMA: before.unbalanceMA / before.system.ctPrimary,
    afterMA: after.unbalanceMA,
    afterSecondaryMA: after.unbalanceMA / after.system.ctPrimary,
    improvement:
...
function exportRecord() {
  ...
  if (lastBest) lastRecord = createRecord(lastBest);
  const summary = [
    [lastRecord.title],
    ...
    ["Before Secondary Unbalanced Current mA", lastRecord.beforeSecondaryMA.toFixed(6)],
    ["After Primary Unbalanced Current mA", lastRecord.afterMA.toFixed(6)],
    ["After Secondary Unbalanced Current mA", lastRecord.afterSecondaryMA.toFixed(6)],
  ...
  link.href = url;
  link.download = `132kv-h-type-48cap-${lastRecord.phase.toLowerCase()}-unbalanced-current-${new Date().toISOString().slice(0, 10)}.csv`;
